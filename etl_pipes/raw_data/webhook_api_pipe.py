...
'''
TODO:
SOURCE IS HARDCODED FOR FETCH_DB_MAX_ID
...
class PullData(object):
    ...
    def get_lead_id_hold(self, start_date, end_date, cnt):
        try:
            ...
            #import pdb;pdb.set_trace()
            get_lead_min_max_id_api =  PullData.API_DI[self.env_set] + f"/LMSWebAPI/Api/partnerLeadCountMinAndMaxId?StartDate={start_date}&EndDate={end_date}"
            payload={}
            headers_di = {
            ...
            try:
                ...
                with eventlet.Timeout(PullData.TIMEOUT):
                    resp = requests.get(get_lead_min_max_id_api,
                                        headers = headers_di
                                       )
            except TimeoutError as e:
                log_data = [{"LogDetails" : f'Webhook API - Pipe (Google,Meta,Whatsapp) TIMEOUT Error {str(e)}', 
                ...
                raise TimeoutError('Webhook Pipe API Error')
            
            if resp.status_code != 200:
                log_data = [{"LogDetails" : f"Error in lead id min max api response status code : {str(resp.status_code)} & resp : {str(resp.json())}", 
                "ScriptName" : str(__file__), "ModuleName" : str(self.__class__.__name__), 
            ...
            if cnt < 3:
                ...
                log_data = [{"LogDetails" : f"last attempt of api hit Error in lead id min max api response status code...", 
                "ScriptName" : str(__file__), "ModuleName" : str(self.__class__.__name__), 
                "SeverityLevel" : 4, "Stage" : "T1", "Status" : "ERROR"}]
                self.log_ops.save_logs(log_data)
        return resp
    ...
    def get_lead_details_hold(self, min_id, max_id, cnt):
        ...
        try:
            ...
            for buck in buckets:
                ...
                get_lead_details_api = PullData.API_DI[self.env_set] + f"/LMSWebAPI/Api/partnerLeadDetailsById?MinId={buck[0]}&MaxId={buck[1]}"
                try:
                    ...
                    try:
                        ...
                        log_data = [{"LogDetails" : f'Webhook API - Get Lead Details - Pipe (Google,Meta,Whatsapp) TIMEOUT Error {str(e)}', 
                "ScriptName" : str(__file__), "ModuleName" : str(self.__class__.__name__), 
                "SeverityLevel" : 5, "Stage" : "T1", "Status" : "ERROR"}]
                        self.log_ops.save_logs(log_data)
                        raise TimeoutError('Timeout Error Webhook -Get Lead Details')
                    if resp.status_code == 200:
    ...
    def read_json(self, filepath):
        if not os.path.exists(filepath):
            log_data = [{"LogDetails" : f"File is Missing at {str(filepath)}", 
            "ScriptName" : str(__file__), "ModuleName" : str(self.__class__.__name__), 
            "SeverityLevel" : 4, "Stage" : "T1", "Status" : "ERROR"}]
            self.log_ops.save_logs(log_data)
    ...
    def fetch_db_max_id(self):
        ...
        try:
            ...
            #query = f"SELECT MAX(cast(LeadID as numeric)) AS MAX_ID FROM dbo.RawLeads WITH (NOLOCK) WHERE Platform !='SAMSUNG' and Platform != 'Meta' and LeadCreationTimestamp > '{min_date}'"
            query = f"EXEC dbo.spEtlWebhookRawFetchDbMaxId @pMinDate = '{min_date}' "
           

            df = self.db_ops.execute_dataframe_query(self.connection, query)
            #if str(df["MAX_ID"][0]).isdigit():
    ...
    def run(self):
        ...
        try:
            ...
            startdate = '20220801' if PullData.UAT == 1 else yesterday_date  
            enddate = today_date
            resp = self.get_lead_id_hold(startdate, enddate, 0)
            if not resp:
                log_data = [{"LogDetails" : f"Error in get lead api id blank resp", 
                "ScriptName" : str(__file__), "ModuleName" : str(self.__class__.__name__), 
                "SeverityLevel" : 4, "Stage" : "T1", "Status" : "ERROR"}]
                self.log_ops.save_logs(log_data)
            ...
            log_data = [{"LogDetails" : f"Successfully ingested {str(leads_df.shape[0])} to RawLeads from WebhookAPI pipe", 
            "ScriptName" : str(__file__), "ModuleName" : str(self.__class__.__name__), 
            "SeverityLevel" : 5, "Stage" : "T1", "Status" : "SUCCESS"}]
            self.log_ops.save_logs(log_data)
        except Exception as e:
            log_data = [{"LogDetails" : f"Error in fetching WebhookAPI  Leads {str(e)}", 
            "ScriptName" : str(__file__), "ModuleName" : str(self.__class__.__name__), 
            "SeverityLevel" : 5, "Stage" : "T1", "Status" : "ERROR"}]
            self.log_ops.save_logs(log_data)
